...
PhysVehicle3D* ModulePhysics3D::rotateVehicle(float angle, PhysVehicle3D* pvehicle) {
	btQuaternion q;
	q.setRotation({ 0,1,0 }, angle);
	btTransform t = pvehicle->vehicle->getChassisWorldTransform();
	t.setRotation(q);

	pvehicle->body->setWorldTransform(t);
	return pvehicle;
}
